export function validateCreateEvent(body) {
  ...
  if (!body.guestPassword || body.guestPassword.length < 4 || body.guestPassword.length > 50) {
    errors.push('guestPassword must be 4-50 characters');
  }
  ...
  if (!body.endDate || !isValidIsoDate(body.endDate)) {
    errors.push('Valid endDate (ISO 8601) is required');
  }
...
export function validateUpdateEvent(body) {
  const errors = [];
  const allowedFields = ['title', 'description', 'footerText', 'welcomeMessage', 'startDate', 'endDate', 'guestPassword'];
